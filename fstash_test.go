...
func createSampleTreeWithGitDir(home string) error {
	d0f1 := filepath.Join(home, "file1.txt")
	d0f2 := filepath.Join(home, "file2.txt")
	...
	gitdir1 := filepath.Join(home, ".git")
	gitDummy1 := filepath.Join(gitdir1, "git-dummy")

	gitdir2 := filepath.Join(dir2, ".git")
	gitDummy2 := filepath.Join(gitdir2, "git-dummy")
	...
	if err := os.MkdirAll(gitdir1, 0777); err != nil {
		return err
	}
	if err := os.MkdirAll(gitdir2, 0777); err != nil {
		return err
	}

	content := []byte(strings.Repeat("a", 100))
	files := []string{d0f1, d0f2, d1f1, d1f2, d2f1, d2f2, d3f1, d3f2, gitDummy1, gitDummy2}
	for _, v := range files {
		if err := ioutil.WriteFile(v, content, 0777); err != nil {
			return err
		}
...
func Test_stash_directory_create_new_stash_ignore_dotgit(t *testing.T) {
	require := require.New(t)
	homeDir1 := filepath.Join(os.TempDir(), randTemp())
	defer func() {
		require.NoError(os.RemoveAll(homeDir1))
	...
	require.Nil(createSampleTreeWithGitDir(homeDir1))
